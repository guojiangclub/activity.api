...
class DefaultCharge extends BaseCharge implements PaymentChargeContract
{
    ...
    public function createWechatCharge($user_id, $channel, $type = 'order', $order_no, $amount, $subject, $body, $ip = '127.0.0.1', $openid = '', $extra = [], $submit_time)
    {
        ...
        $chargeData = [
            ...
            'attach' => json_encode(array('user_id' => $user_id, 'order_sn' => $order_no, 'type' => $type, 'channel' => $channel), true)
        ];

        switch ($channel) {
            case 'wx_pub_qr':
                $config['notify_url'] = $config['notify_url'] . '/wx/'.$type;
            ...
            case 'wx_lite':
                $config = $this->getConfig('miniapp', $type);
                //异步回调地址
                $config['notify_url'] = $config['notify_url'] . '/wx_lite/' . $type;
                /*$user = request()->user();
    ...
    public function getConfig($type, $order_type = 'order')
    {
        $config = [];
        switch ($type) {
            case 'alipay':
    ...
    public function queryByOutTradeNumber($order_no)
    {
        ...
        switch ($paymentLog->channel) {
            ...
            case 'wx_pub_qr':
            case 'wx_lite':
                return $this->queryWechatByOutTradeNumber($paymentLog->transcation_order_no, $paymentLog->channel, $paymentLog->type);
    ...
    protected function queryWechatByOutTradeNumber($order_no, $channel, $type)
    {
        $config = $this->getConfig('wechat');
        ...
        if ($data['return_code'] == 'FAIL') return [];

        if ($data['result_code'] == 'FAIL') return [];

        if ($data['trade_state'] != 'SUCCESS') return [];
