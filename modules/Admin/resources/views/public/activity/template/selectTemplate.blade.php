<div class="row padding-top-7 items_control">
    ...
    <div class="col col-md-3">
        <select class="form-control selectOptions_{{$field['index']}}">
            <option>请选择</option>
            @if(!empty($field['options']))
                @foreach($field['options'] as $option)
                    <option class="select_options_{{$field['index']}}_{{$option['index']}} select_options_delete_{{$field['index']}}_{{$option['index']}}">{{$option['name']}}</option>
                @endforeach
            @endif
        </select>
        @if(!empty($field['options']))
            @foreach($field['options'] as $option)
                <input type="hidden" class="select_options_{{$field['index']}}_{{$option['index']}}" name="activityForm[input_{{$field['index']}}][options][name][]" value="{{$option['name']}}" />
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <input type="text" class="form-control input_{{$field['index']}}_title" value="{{$field['title']}}" placeholder="下拉选择框" oninput="updateTitle('input_{{$field['index']}}_title', this)">
                        </div>
                    </div>
                    @if(!empty($field['options']))
                        @foreach($field['options'] as $option)
                            <div class="form-group select_options_delete_{{$field['index']}}_{{$option['index']}}">
                                <div class="col-md-3"></div>
                                <div class="col-md-6">
                                    <input type="text" class="form-control" value="{{$option['name']}}" oninput="updateTitle('select_options_{{$field['index']}}_{{$option['index']}}', this)" placeholder="选项">
                                </div>
                                <div class="col-md-1"><a class="btn btn-xs btn-danger select_options_delete" data-num="{{$field['index']}}_{{$option['index']}}" style="cursor:pointer; margin-top:13px;"><i class="fa fa-trash"></i></a></div>
                            </div>
                        @endforeach
                    @endif
                    <div class="form-group">
                        <div class="col-md-3"></div>
                        <div class="col-md-6">
                            <a class="btn btn-success select_add_option" data-type="selectOptions_{{$field['index']}}" data-index="{{$field['index']}}">添加选项</a>
                        </div>
                    </div>
                </div>
