...
class ActivityOrderController extends Controller
{
	...
	public function index()
	{
		...
		if (request('excel') == 1 || request('excel') == 2 || request('excel') == 3) {
			if (request('excel') == 1) {
				$where[] = ['status', '=', $status];
			}
			...
			if (count($orders)) {
				...
				foreach ($orders as $order) {
					...
					if (request('excel') == 3 && !empty($fields)) {
						...
						if ($answer) {
							...
							foreach ($fields as $field) {
								...
								if (!isset($answer[$field['name']])) {
									...
									} elseif ($field['name'] == 'id_card' || preg_match("/^([\d]{17}[xX\d]|[\d]{15})$/", $answer[$field['name']])) {
	...
	protected function getData($object, $where, $limit = 15)
	{
		...
		foreach ($where as $key => $item) {
			if ($item[0] != 'order_no' && $item[1] == 'like') {
				$search = $item;
			}
		}
	...
	public function paymentOptions(Request $request)
	{
		...
		if (count($payments) > 0) {
			foreach ($payments as $payment) {
				$title = $payment->title;
				if ($payment->title == '' && $payment->type == 3) {
					$title = '通行证活动';
				}

				if ($payment->title == '' && $payment->type == 4) {
					$title = '线下互动';
				}
