<div class="row padding-top-7 items_control">
    <label class="col-md-2 control-label input-title" @if($field['name']!='certificate_type') data-toggle="modal" data-target="#input_{{$field['index']}}_modal" @endif><span class="input_{{$field['index']}}_title" title="点击修改">{{$field['title']}}</span>：</label>
    <div class="col col-md-3">
        <div class="form-group">
            <div class="radio radioOptions_{{$field['index']}}">
                @if(!empty($field['options']))
                    @foreach($field['options'] as $option)
                        <label class="radio_options_delete_{{$field['index']}}_{{$option['index']}}">
                            <input type="radio" disabled>
                            <span class="radio_options_{{$field['index']}}_{{$option['index']}}">
                                {{$option['name']}}
                            </span>
                            <input type="hidden" class="radio_options_{{$field['index']}}_{{$option['index']}}" name="activityForm[input_{{$field['index']}}][options][name][]" value="{{$option['name']}}" />
                            <input type="hidden" name="activityForm[input_{{$field['index']}}][options][index][]" value="{{$option['index']}}" />
                            @if($field['name']=='certificate_type')
                                <input type="hidden" name="activityForm[input_{{$field['index']}}][options][value][]" value="{{$option['value']}}" />
                            @endif
                        </label>
                    @endforeach
                @endif
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <div class="col-md-6"><input type="text" class="form-control input_{{$field['index']}}_title" value="{{$field['title']}}" placeholder="单选按钮" oninput="updateTitle('input_{{$field['index']}}_title', this)"></div>
                    </div>
                    @if(!empty($field['options']))
                        @foreach($field['options'] as $option)
                            <div class="form-group radio_options_delete_{{$field['index']}}_{{$option['index']}}">
                                <div class="col-md-3"></div>
                                <div class="col-md-6">
                                    <input type="text" class="form-control" value="{{$option['name']}}" oninput="updateTitle('radio_options_{{$field['index']}}_{{$option['index']}}', this)" placeholder="选项">
                                </div>
                                <div class="col-md-1">
                                    <a class="btn btn-xs btn-danger radio_options_delete" data-num="{{$field['index']}}_{{$option['index']}}" style="cursor:pointer; margin-top:13px;"><i class="fa fa-trash"></i></a>
                                </div>
                            </div>
                        @endforeach
                    @endif
                    <div class="form-group">
                        <div class="col-md-3"></div>
                        <div class="col-md-6">
                            <a class="btn btn-success radio_add_option" data-type="radioOptions_{{$field['index']}}" data-index="{{$field['index']}}">添加选项</a>
                        </div>
                    </div>
                </div>
