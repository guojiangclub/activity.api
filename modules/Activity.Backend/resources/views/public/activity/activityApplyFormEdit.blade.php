<div class="form-group" id="activityForm">
    ...
    <div class="col-md-10">
        <div class="vim-panel col vim-margin-top activity-form">
            ...
            <div class="row"><div class="col-md-8 line-div"></div></div>--}}
            @if(!empty($fields))
                @foreach($fields as $key=>$field)
                    @if($field['type']=='text')
                        @include('activity::public.activity.template.textTemplate')
                    @elseif($field['type']=='textarea')
                        @include('activity::public.activity.template.textAreaTemplate')
                    @elseif($field['type']=='select')
                        @include('activity::public.activity.template.selectTemplate')
                    @elseif($field['type']=='radio')
                        @include('activity::public.activity.template.radioTemplate')
                    @elseif($field['type']=='checkbox')
                        @include('activity::public.activity.template.checkboxTemplate')
                    @elseif($field['type']=='range')
                        @include('activity::public.activity.template.rangeTemplate')
                    @else
                        @include('activity::public.activity.template.fileTemplate')
                    @endif
                @endforeach
            @endif
        </div>
