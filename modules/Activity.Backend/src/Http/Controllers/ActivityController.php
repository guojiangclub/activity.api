...
class ActivityController extends Controller
{
    ...
    public function store()
    {
        ...
        if ('CHARGING' == $input['fee_type'] && (empty($title) || empty($point) || empty($price) || empty($limit))) {
            return response()->json(['status' => false, 'error_code' => 0, 'error' => '必须添加一张电子票']);
        }

        if ('PASS' == $input['fee_type']) {
        ...
        try {
            ...
            } elseif ('OFFLINE_CHARGES' == $input['fee_type']) {
    ...
    public function update($id)
    {
        ...
        if (0 == $activity_status and 1 == $input['status']) {
            $input['published_at'] = Carbon::now();
        }
        ...
        if ('CHARGING' == $activity->fee_type && (empty($title) || empty($point) || empty($price) || empty($limit))) {
            return response()->json(['status' => false, 'error_code' => 0, 'error' => '必须添加一张电子票']);
        }
        ...
        try {
            ...
            if ('CHARGING' == $input['fee_type']) {
                ...
                foreach ($paymentPostIds as $key => $id) {
                    if (in_array($id, $paymentIds)) {
                        $paymentUpdateIds[] = $id;
                    } elseif (!in_array($id, $paymentIds) && false !== strpos($id, 'new_')) {
                        $paymentCreateIds[$key] = substr($id, 4);
                    }
                }

                foreach ($paymentIds as $id) {
                    if (!in_array($id, $paymentPostIds) && false === strpos($id, 'new_')) {
            ...
            //修改线下支付金额
            if ('OFFLINE_CHARGES' == $input['fee_type']) {
    ...
    protected function validateForm($fee_type)
    {
        ...
        $validator->sometimes('package_get_time', 'before:starts_at', function ($input) {
            return 1 == $input['send_message'] && $input['package_get_time'] > $input['starts_at'];
        });
    ...
    public function rewards($id)
    {
        ...
        $members = $members->filter(function ($member) use ($id) {
            if (1 == $member->rewarded and $point = $this->point->findWhere([
